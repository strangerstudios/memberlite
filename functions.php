...
function memberlite_init_styles() {
	if ( is_rtl() ) {
		wp_enqueue_style( 'memberlite_rtl', MEMBERLITE_URL . '/build/css/main.rtl.css', array(), MEMBERLITE_VERSION );
		wp_enqueue_style( 'memberlite_style', get_stylesheet_uri(), array( 'memberlite_rtl' ), MEMBERLITE_VERSION );
	} else {
		wp_enqueue_style( 'memberlite_main_style', MEMBERLITE_URL . '/build/css/main.css', array(), MEMBERLITE_VERSION );
		wp_enqueue_style( 'memberlite_style', get_stylesheet_uri(), array( 'memberlite_main_style' ), MEMBERLITE_VERSION );
	}
...
function memberlite_widgets_init() {
	...
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-3',
			'description'   => __( 'Depending on your header variation, this is a spot you can put extra things.', 'memberlite' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
	...
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-4',
			'description'   => __( 'This is a legacy method to customize the site footer. For more control, create a new Footer Variation and assign it in Customizer settings.', 'memberlite' ) . ' <a href="https://memberlitedev.local/wp-admin/edit.php?post_type=memberlite_footer" target="_blank">' . __( 'Edit Footer Variations', 'memberlite' ) . '</a>',
...
function memberlite_menus( $items, $args ) {
	...
	if ( $args->theme_location == 'member' || $args->theme_location == 'member-logged-out' || ( substr( $args->theme_location, -strlen( '-member' ) ) === '-member' ) ) {
		if ( is_user_logged_in() && defined( 'PMPRO_VERSION' ) && pmpro_hasMembershipLevel() ) {
			...
			$items .= '<li class="menu-item menu-item-depth-0"><a href="' . esc_url( wp_logout_url( memberlite_logout_redirect_to() ) ) . '">' . esc_html__( 'Log Out', 'memberlite' ) . '</a></li>';
		} elseif ( is_user_logged_in() ) {
			// user is logged in and does not have a membership level
			$items = '<li class="menu-item menu-item-depth-0"><a href="' . esc_url( wp_logout_url( memberlite_logout_redirect_to() ) ) . '">' . esc_html__( 'Log Out', 'memberlite' ) . '</a></li>';
		} else {
			// not logged in
			$items .= '<li class="menu-item menu-item-depth-0"><a href="' . esc_url( wp_login_url( memberlite_login_redirect_to() ) ) . '">' . esc_html__( 'Log In', 'memberlite' ) . '</a></li>';
			...
			if ( ! empty( $show_register_link ) ) {
				$items .= '<li class="menu-item menu-item-depth-0"><a href="' . esc_url( wp_registration_url() ) . '">' . esc_html__( 'Register', 'memberlite' ) . '</a></li>';
			}
		}
	}
...
function memberlite_member_menu_cb( $args ) {
	...
	if ( empty( $link_before ) ) {
		$link_before = '<li class="menu-item menu-item-depth-0">';
	}
	if ( empty( $link_after ) ) {
		$link_after = '</li>';
	}
